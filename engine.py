...
def handle_github_automation():
    ...
    # Si on est sur un push direct sur main, on ne fait pas de PR, on laisse le workflow commit
    if event == 'push':
        print("Mode Push détecté : Mutation appliquée localement.")
        return

    # Mode Automatique (Schedule/Dispatch) : Création de PR
    try:
    ...
    try:
        ...
        payload = json.dumps({
            ...
            'head': branch_name,
            'base': base_branch,
            'body': 'Mutation algorithmique vers le ratio d\'or.'
        }).encode('utf-8')
