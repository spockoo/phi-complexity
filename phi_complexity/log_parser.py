...
CATALOGUE_SIGNATURES: List[PatternSignature] = [
    ...
    PatternSignature(
        ...
        priority=1,
        hint="Ref Git introuvable ou repo absent/privé (branche/tag supprimée, repo 404).",
        mutation="Vérifier l'existence de la ref cible (branche/tag) ou rebaser sur main avant de relancer la CI.",
    ),
    PatternSignature(
        ...
        confidence_base=0.75,
        priority=1,
        hint="Échec checkout Git (erreur générique checkout/fetch).",
        mutation="Collecter les logs détaillés de checkout, nettoyer le workspace puis relancer la CI.",
    ),
    PatternSignature(
