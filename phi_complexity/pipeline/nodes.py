...
from .orchestrator import PipelineNode, PipelineSignal
    ...
    async def execute(self):
        while True:
            ...
            if signal.action == "review_plan":
                logger.info("[ValidationNode] Analyse du contrat pour failles conceptuelles...")
        ...
        while True:
            ...
            if signal.action in ["approved_plan", "quality_rejected"]:
                if signal.action == "quality_rejected":
                    logger.warning("[ImplementationNode] Rejet Mathématique ! Réécriture du code...")
                    # LLM: Corriger les problèmes spécifiques de radiance passés dans signal.data
                else:
                    logger.info("[ImplementationNode] Synthèse du code selon le plan...")
    ...
    async def execute(self):
        while True:
            ...
            if signal.action == "code_ready":
                logger.info("[QualityGateNode] Évaluation de la Radiance Spatiale (φ)...")
                # Fausse cible pour le squelette, le vrai code cible le workspace temporaire
                radiance_score = 100.0  
                
                try:
                    calculateur = CalculateurRadiance()
                    # Exemple minimal : radiance_score = calculateur.calculer_radiance(target)
                except NameError:
                    pass

                # LE VERROU DU NOMBRE D'OR
                if radiance_score < 80.0:
                    logger.error(f"[QualityGateNode] REJET : Radiance inacceptable ({radiance_score} < 80.0)")
                    await self.send_signal(
                        target_node=self.context.get("implementation_node"),
                        action="quality_rejected",
                        data={"radiance": radiance_score, "issues": ["Entropie excessive", "Boucles asymétriques"]}
                    )
                else:
                    logger.info(f"[QualityGateNode] APPROBATION : Radiance Hermétique ({radiance_score} >= 80.0)")
